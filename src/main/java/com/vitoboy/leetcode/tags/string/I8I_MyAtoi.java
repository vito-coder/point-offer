 ...
 * @date 2021/8/13
 */
public class I8I_MyAtoi {
    public static void main(String[] args) {
        I8I_MyAtoi atoi = new I8I_MyAtoi();
        System.out.println(atoi.myAtoi("42"));
        System.out.println("expect is : 42");
        System.out.println(atoi.myAtoi("-42"));
        System.out.println("expect is : -42");
        System.out.println(atoi.myAtoi("4193 with words"));
        System.out.println("expect is : 4193");
    ...
    /**
     * 				解答成功:
     * 				执行耗时:2 ms,击败了92.69% 的Java用户
     * 				内存消耗:38.5 MB,击败了44.00% 的Java用户
     *
     * 时间复杂度: O(N)
     * 空间复杂度: O(1)
     *
    ...
    public int myAtoi(String s) {
        long tmp = 0L;
        boolean begin = false, mine = false;
        for (int i = 0, len = s.length(); i < len; i++) {
            if (s.charAt(i) == ' ' && !begin) continue;
            if (s.charAt(i) == '-' && !begin) {
                begin = true;
                mine = true;
                continue;
            ...
            if (s.charAt(i) >= '0' && s.charAt(i) <= '9') {
                ...
                if (tmp > Integer.MAX_VALUE && !mine) return Integer.MAX_VALUE;
                if (mine && tmp*(-1) < Integer.MIN_VALUE) return Integer.MIN_VALUE;
            } else {
                if (tmp > Integer.MAX_VALUE && !mine) return Integer.MAX_VALUE;
                if (mine && tmp*(-1) < Integer.MIN_VALUE) return Integer.MIN_VALUE;
                if (mine) return (int) (tmp*(-1));
                else return (int)tmp;
            }
        }
        if (tmp > Integer.MAX_VALUE && !mine) return Integer.MAX_VALUE;
